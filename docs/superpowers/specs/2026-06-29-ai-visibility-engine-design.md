...
3. **Resolve + detect** (qckbot `ai-prompt-check.ts:43-122`): redirect-resolve each returned domain (SSRF-guarded, reuse `lib/fetch.ts safeFetchText`, read final host). Brand appears if any resolved domain === target domain; record its `rank` as position + its `reason` as snippet. Competitors = other resolved domains, `isDeniedCompetitor`-filtered, max 5 per prompt.
